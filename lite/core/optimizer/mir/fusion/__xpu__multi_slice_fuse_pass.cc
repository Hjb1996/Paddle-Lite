...
class XPUMultiSliceFuser : public FuseBase {
 public:
  explicit XPUMultiSliceFuser(int slice_cnt = 1)
      ...
      auto* slice =
        OpNode(slice_op_name, "slice")
            ...
            ->assert_op_attr_satisfied_specified_value<std::vector<int>, int>(
                "starts",
                i,
                [](const std::vector<int>& attr, const int val) { return attr[0] == val; })
            ->assert_op_attr_satisfied_specified_value<std::vector<int>, int>(
                "ends",
                i,
                [](const std::vector<int>& attr, const int val) { return attr[0] == val+1; })
            ->AsIntermediate();
 ...
 public:
  void Apply(const std::unique_ptr<SSAGraph>& graph) override {
    ...
    for (auto slice_cnt : slice_cnts) {
      fusion::XPUMultiSliceFuser fuser(slice_cnt);
      fuser(graph.get());
    }
